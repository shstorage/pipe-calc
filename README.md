...
ASME B31.1 / B31.3 기준 **배관 최소 요구 두께 계산 웹 애플리케이션**입니다.
...
- **B31.1 / B31.3 두께 계산** — 공식에 따라 전 DN 사이즈 일괄 계산
- **허용응력 선형 보간** — 입력 온도가 테이블 사이 값이면 자동 보간
- **배관 스케줄 DB** — B36.10 / B36.19, DN 15~600 내장
- **허용응력 DB** — A106 Gr.B / A312 TP304, 상온~450℃ 내장
...
- `seed_pipe.py` — ASME B36.10 / B36.19 배관 스케줄 274건 입력
- `seed_allowable.py` — A106 Gr.B / A312 TP304 허용응력 40건 입력
...
| 좌측 | 우측 |
|------|------|
| 적용 코드 (B31.1 / B31.3) 선택 | DN 15 ~ DN 600 전 사이즈 계산 결과 |
| 재질, 설계 압력, 설계 온도 입력 | 요구 두께 및 최소 만족 스케줄 표시 |
| 부식 허용치, 고급 옵션 (E·Y·W) | 만족하는 스케줄 없을 경우 "해당 없음" 표시 |
